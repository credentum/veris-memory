...
from ..storage.enhanced_storage import (
    EnhancedStorageOrchestrator, initialize_storage_orchestrator
)
...
@click.option('--source', type=click.Choice(['qdrant', 'neo4j', 'redis', 'all']), 
              default='all', help='Source system to migrate from')
@click.option('--target', type=click.Choice(['text']), default='text',
    ...
    async def _run_backfill():
        ...
        try:
            ...
            if source in ['redis', 'all']:
                try:
                    kv_store = KVStore(config_path=config_path)
            ...
            click.echo(f"   Success rate: {(result_job.success_count / result_job.processed_count * 100):.1f}%" if result_job.processed_count > 0 else "   Success rate: 0%")
            ...
            click.echo(f"\n✨ Migration completed successfully!" if result_job.status == MigrationStatus.COMPLETED else f"\n⚠ Migration completed with issues: {result_job.status}")
...
def status(config_path, test_mode):
    ...
    async def _check_status():
        ...
        try:
            ...
            backends['Text Search (BM25)'] = f"✓ Available ({stats['document_count']} documents, {stats['vocabulary_size']} terms)"
        except Exception as e:
        ...
        if 'Text Search (BM25)' in backends and backends['Text Search (BM25)'].startswith('✓'):
            try:
                ...
                click.echo(f"   Avg doc length: {stats.get('average_document_length', 0):.1f} tokens")
                
                if stats.get('top_terms'):
                    click.echo("   Top terms:")
                    for term_info in stats['top_terms'][:5]:
                        click.echo(f"     • '{term_info['term']}': {term_info['document_frequency']} documents")
...
@click.option('--backend', type=click.Choice(['text', 'vector', 'graph', 'hybrid']), 
              default='hybrid', help='Backend to use for search')
