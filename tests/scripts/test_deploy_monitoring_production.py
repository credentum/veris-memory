...
""")
    ...
    def test_resource_limits_configuration(self, temp_env_file):
        """Test resource limits are appropriate for 64GB Hetzner system."""
        with open(temp_env_file, 'r') as f:
            content = f.read()
        ...
        assert 8 <= cpu_cores <= 16, f"CPU limit {cpu_cores} should be reasonable for Hetzner system"
